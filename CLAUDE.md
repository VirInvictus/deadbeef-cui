...
9. If the rebuild left `playlist_dirty` set and real data has been seen (`initial_sync_done`), re-mirrors the viewer playlist via the chunked fill — WITHOUT making it current (see §6.15). This is the reversal of the v1.2.4 deferral: the panes must not go current while the playlist tab keeps mirroring old data. The cache-hit early-return above arms nothing, so no-op events cost nothing.
...
`update_playlist_from_cui(cw, sync, make_current)` mirrors the filtered library into the viewer on the idle queue in ~75 ms chunks (async mode; v1.3.7): a whole-library mirror is ~1 s of `pl_item_copy`, and doing it synchronously froze the UI on every `[All]` transition (measured live). The fill's DFS frames hold pointers INTO `cw->cached_tree`, and its cursor refs tracks — so every one of these MUST call `cui_fill_cancel(cw)` first:
...
The chunk itself re-checks `shutting_down` and `g_list_find(all_cui_widgets, cw)` every tick — the §6.3 uniform guard. `playlist_dirty` clears ONLY at fill completion, so an aborted fill always leaves the dirty flag set and the next activation rebuilds synchronously. `activate_row` deliberately uses the SYNC path with `make_current=1`: `DB_EV_PLAY_NUM` needs the playlist complete before it fires, and activation legitimately makes the viewer current. 

**When the fill fires (the v1.2.4 deferral is REVERSED):** every real `update_tree_data` rebuild ends by re-mirroring the playlist when the rebuild left it dirty and real data has been seen (`initial_sync_done`) — this covers library events, search keystrokes, and config-driven rebuilds, and the first successful build doubles as the launch pre-fill (dirty starts set at widget creation). The deferral existed because a refill was a ~1 s synchronous copy; the chunked fill removed that cost, and a deferred mirror meant the populated tab held stale data until a facet click or relaunch. The modification-index cache-hit arms nothing, so no-op events are free.

**`make_current` discipline:** only interaction-driven fills (facet selection cascade, `activate_row`) may `plt_set_curr` the viewer. Programmatic refills pass `make_current=0` — the same no-steal rule as the v1.3.5 selection fix; v1.3.7's launch pre-fill violated it and switched the user's restored current playlist on every startup. Locked in by the `/cui/fill/*` tests (completion, cancel-on-tree-free, sync supersedes, refill-after-rebuild, no-steal-on-programmatic-refill).
...
3. **`roadmap.md`**: flip `[ ]` to `[x]` for completed items. Add new entries to the appropriate phase, or to "Deferred (v2.0+)" if they're scope-creep. Release records live in `patchnotes.md` ONLY: do not add per-release summary blocks to the roadmap (DECIDED 2026-09-18, Brandon; the v1.3.5–v1.3.7 "Release vX (SHIPPED)" blocks predate this and stay as history).
...
- **gh release quirks (learned cutting v1.3.5, 2026-09-15):** `gh release
  create --notes-from-tag` is REJECTED in combination with `--repo` (the
  workflow derives the repo from GITHUB_REPOSITORY instead), and without
  `--title` the release name is left EMPTY (v1.3.4 and v1.3.5 both needed a
