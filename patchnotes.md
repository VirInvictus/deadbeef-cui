...
**Eliminated Continuous Tree Rebuilds.** `DB_EV_TRACKINFOCHANGED` fires on every play, skip, pause, and playqueue mutation — not just metadata edits — which caused the plugin to rebuild the entire library tree roughly every half-second during playback, scrolling every facet back to the top and clobbering the Library Viewer playlist. The handler has been removed entirely. Automatic refreshes only flow from the medialib's own `CONTENT_DID_CHANGE` events (folder scans, startup), never from playback noise.
**Manual "Sync library" Menu Item.** Right-clicking any facet row now offers a **Sync library** option that triggers a `medialib_plugin->refresh()` on demand. Use this after editing tags in the track properties dialog to pull the new metadata into the facets without restarting DeaDBeeF. Because the refresh is user-initiated, the ensuing tree rebuild isn't a surprise — the alternative (auto-refreshing on every tag-ish event) repeatedly reset scrollbars, blew away selections, and wiped the playlist mid-playback.
