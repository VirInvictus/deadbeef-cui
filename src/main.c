...
typedef struct {
    ...
    GtkListStore *store_genre, *store_artist, *store_album;
    GtkWidget *tree_genre, *tree_artist, *tree_album;
    int listener_id; guint idle_id;
    ddb_medialib_item_t *cached_tree;
    const ddb_medialib_item_t *sel_genre_node, *sel_artist_node, *sel_album_node;
...
static void aggregate_recursive_multi(GtkListStore *store, const ddb_medialib_item_t *node, int cl, int tl, cui_widget_t *cw, GHashTable *seen) {
    if (cl == tl) {
        ...
        if (t && !g_hash_table_contains(seen, t)) {
            ...
            gtk_list_store_set(store, &iter, 0, t, -1); g_hash_table_add(seen, g_strdup(t));
        }
        return;
    }
...
static void populate_list_multi(GtkListStore *store, int tl, cui_widget_t *cw, const char *all_t) {
    ...
    if (all_t) {
        GtkTreeIter iter; gtk_list_store_append(store, &iter);
        gtk_list_store_set(store, &iter, 0, all_t, -1); g_hash_table_add(seen, g_strdup(all_t));
    }
...
static const ddb_medialib_item_t *find_node_by_text(const ddb_medialib_item_t *p, const char *st) {
    ...
    while (c) {
        const char *t = medialib_plugin->tree_item_get_text(c);
        if (t && !strcmp(t, st)) return c;
        c = medialib_plugin->tree_item_get_next(c);
    }
    return NULL;
}
...
static void ml_listener_cb(ddb_mediasource_event_type_t ev, void *ud) {
    ...
    if (!shutting_down && (ev == DDB_MEDIASOURCE_EVENT_STATE_DID_CHANGE || ev == DDB_MEDIASOURCE_EVENT_CONTENT_DID_CHANGE)) {
        ...
        cw->idle_id = g_idle_add(repopulate_ui_idle, ud);
    }
}
...
static void on_artist_changed(GtkTreeSelection *s, gpointer ud) {
    ...
    if (gtk_tree_selection_get_selected(s, &m, &iter)) {
        ...
        if (a && strcmp(a, ALL_ARTISTS)) {
            cw->sel_artist_text = g_strdup(a);
            if (cw->sel_genre_node) cw->sel_artist_node = find_node_by_text(cw->sel_genre_node, a);
        }
        g_free(a);
    }
    populate_list_multi(cw->store_album, 3, cw, ALL_ALBUMS);
...
static void on_album_changed(GtkTreeSelection *s, gpointer ud) {
    ...
    if (gtk_tree_selection_get_selected(s, &m, &iter)) {
        gchar *a; gtk_tree_model_get(m, &iter, 0, &a, -1);
        if (a && strcmp(a, ALL_ALBUMS)) {
            cw->sel_album_text = g_strdup(a);
            if (cw->sel_artist_node) cw->sel_album_node = find_node_by_text(cw->sel_artist_node, a);
        }
        g_free(a);
    }
    update_playlist_from_cui(cw);
...
static GtkWidget *create_column(const char *title, GtkListStore **os, GtkWidget **ot) {
    ...
    GtkWidget *tree = gtk_tree_view_new_with_model(GTK_TREE_MODEL(store)); g_object_unref(store);
    gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(tree), TRUE);
    GtkCellRenderer *r = gtk_cell_renderer_text_new();
    GtkTreeViewColumn *c = gtk_tree_view_column_new_with_attributes(title, r, "text", 0, NULL);
...
static ddb_gtkui_widget_t *cui_create_widget(void) {
    ...
    GtkWidget *p1 = gtk_paned_new(GTK_ORIENTATION_HORIZONTAL); GtkWidget *p2 = gtk_paned_new(GTK_ORIENTATION_HORIZONTAL);
    GtkWidget *cg = create_column("Genre", &cw->store_genre, &cw->tree_genre);
    GtkWidget *ca = create_column("Album Artist", &cw->store_artist, &cw->tree_artist);
    GtkWidget *cb = create_column("Album", &cw->store_album, &cw->tree_album);
    gtk_paned_pack1(GTK_PANED(p2), ca, TRUE, FALSE); gtk_paned_pack2(GTK_PANED(p2), cb, TRUE, FALSE);
    gtk_paned_pack1(GTK_PANED(p1), cg, TRUE, FALSE); gtk_paned_pack2(GTK_PANED(p1), p2, TRUE, FALSE);
    ...
    GtkTreeSelection *sg = gtk_tree_view_get_selection(GTK_TREE_VIEW(cw->tree_genre));
    g_signal_connect(sg, "changed", G_CALLBACK(on_genre_changed), cw);
    GtkTreeSelection *sa = gtk_tree_view_get_selection(GTK_TREE_VIEW(cw->tree_artist));
    g_signal_connect(sa, "changed", G_CALLBACK(on_artist_changed), cw);
    GtkTreeSelection *sb = gtk_tree_view_get_selection(GTK_TREE_VIEW(cw->tree_album));
    g_signal_connect(sb, "changed", G_CALLBACK(on_album_changed), cw);
